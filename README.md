...
node bin/clipforge.mjs create --topic "在家手冲咖啡" --quality hd --bgm --bgm-volume 24   # 一句话出片，配乐强度 24%，回填 videoUrl
...
**下一步（在现有真实能力上继续加深）**
- ✅ **本地文字剪辑**：导入已有视频后用 Transformers.js 在浏览器本地转写，删词、去静音、字幕校对、剪辑计划 JSON 导入/导出、批量下载 OTIO / EDL / CSV 时间线，原片和历史版本不覆盖。
- ✅ **长视频切片工作台**：按原话搜索和目标时长生成候选片段，试听后确认再渲染；任务可取消、续跑、重试，网页 / CLI / MCP 共用同一套计划。
- ✅ **跨平台声音控制**：配乐强度可调，带旁白时默认自动闪避，后端对外部调用统一限制在安全音量范围，风格包也能保存这项设置。
- ✅ **节奏标记**：转写句子起点会映射到 OTIO Marker、EDL 注释和 CSV 节奏列，交给专业剪辑软件后无需重新转写即可对齐对白与音乐。
- ✅ **字幕文字轨道**：转写句子会写入 OTIO 的可编辑 C1 字幕轨道，并同步出现在 EDL 注释和 CSV 字幕列中。
- ✅ **音轨清单 sidecar**：每次合成会保存旁白、原生音频和 BGM 的 A1/A2/A3 时间映射，CLI / MCP 可通过成片的 `timelineUrl` 读取，来源只保留文件名，不暴露本机路径。
- ✅ **可编辑音轨导出**：CLI / MCP 传入 `--audio-stems` / `audioStems: true` 后，sidecar 会附带旁白和 BGM WAV 下载地址；默认关闭，不增加普通合成耗时。
- [ ] 多轨时间线（对白 / BGM / 音效 / 字幕分轨）
- [ ] 数字人口型（fal.ai Lipsync）与更细粒度的区域修复
